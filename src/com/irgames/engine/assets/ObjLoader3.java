 ...
 */
public class ObjLoader3 extends ModelLoader<ObjLoader3.ObjLoaderParameters> {
    ...
    public ObjLoader3(FileHandleResolver resolver) {
        super(resolver);
    }
    ...
    public IRNode getModel(FileHandle fh) {
        ModelData mdata = loadModelData(fh, false);

        Model mod = new Model(mdata, new TextureProvider.FileTextureProvider());
        ModelInstance mins = new ModelInstance(mod);
        IRNode irn = new IRNode(mins);

        File f = new File(fh.parent().path() + File.separator + fh.nameWithoutExtension() + ".irmat");
        if (f.exists()) {
            //JOptionPane.showMessageDialog(null, "Material exists!");
            try {
                IRMatSaveLoad.load(irn, fh.parent().path() + File.separator + fh.nameWithoutExtension() + ".irmat");
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (tokens[0].length() == 0) {
                    ...
                    } else if (key.equals("kd") || key.equals("ks")) // diffuse or specular
                    {
                        ...
                        texFilename = Gdx.files.absolute(tokens[1]).path();//file.parent().child(tokens[1]).path();
                    }
                }
                mtlFile += line + "\n";
            }
